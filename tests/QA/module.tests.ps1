...
BeforeDiscovery {
    # DeskPilot is an application module: it exports a single public command
    # (Start-DeskPilot) and keeps its ~60 internal helpers Private. The
    # per-function quality gates below (unit-test-exists, Script Analyzer,
    # comment-based help) therefore run against the EXPORTED surface only; the
    # Private helpers are covered collectively by
    # tests/Unit/DeskPilot.Helpers.Tests.ps1. Enumerating exported commands from
    # outside the module scope yields just the public set.
