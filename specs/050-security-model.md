...
| T9c | An expensive Git read stalls the accept loop. | `Get-DpGitChanges` reports an untracked **folder** as one entry for the repository-wide call (only a path-filtered call expands file by file), applies its 500-file cap **while building** the list rather than afterwards, and measures only the files it reports. `Measure-DpFileLine` reads at most 2 MiB per file. The SPA caches the Git bar's change count and re-reads it at most every 20 s. |
