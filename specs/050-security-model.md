...
| T4 | The cached Copilot OAuth token is read by another user on a shared machine. | Inherited Engine behaviour (clear-text at `~/.copilot-demo-token`); DeskPilot documents it and recommends single-user machines; encrypted storage tracked upstream. |
