...
| FR-C13 | C | Let the user dictate a prompt by voice (speech-to-text) and have an assistant Message read aloud (text-to-speech), using the browser's built-in speech APIs with no extra dependency. The controls are shown only when the browser supports them. The user can choose the spoken language (English and German variants, or Auto to follow the browser); read-aloud selects a voice matching that language. |
...
| FR-T10a | S | Name each file **while the Turn is still writing it**, in that same card, without requiring a Git repository or the model's thinking to be shown. The rows state intent (no counts, no diff) and are replaced by the reviewed card when the Turn ends; when there is nothing to review they remain, so a stopped Turn still says what it wrote. |
| FR-T11 | S | Let the user open any changed file in a **Diff viewer** showing a unified diff with both old and new line numbers, step through the whole change set, and undo a single file. A pending DeskPilot change is diffed against its snapshot; a new file is shown as all additions; a file with no text to compare shows the picture when it is an image format a browser can draw, and says so in words otherwise. |
...
| ID | Priority | Requirement |
| --- | --- | --- |
| FR-PA1 | S | When approval is active, run `Invoke-Shp` with **`-DisableTerminal`** and register a DeskPilot-owned Tool named **`run_terminal_command`** instead. The name must not be a built-in: the Engine dispatches its built-ins from literal `switch` clauses and reaches registered Tools only through that switch's `default`, so a Tool named `run_command` would be advertised to the Model and then never invoked while the built-in ran the command ungated. `-DisableTerminal` then removes the built-in from the offered tool set, and the Engine refuses to dispatch a built-in it did not offer, so a stray `run_command` call fails closed instead of running beside the gate. DeskPilot probes the Engine for that refusal at registration and fails loudly without it, rather than reporting an approval gate it cannot honour. Approval is active only when `perCallApproval`, the Terminal Permission and the Your Tools Permission are all on. |
| FR-PA2 | M | **Block before the side effect, never after it.** The owned Tool parks on a dedicated approval bridge - a second instance of the rendezvous `ask_questions` uses - so no command has run when the question reaches the user, and the Turn resumes where it stopped. The bridge is separate so a question and an approval cannot evict each other from the single slot. |
| FR-PA3 | M | Ask **only about what is not recognised.** A command matching the shipped safe-list (read-only `git` subcommands, PowerShell readers, `ls`/`cat`/`head`/`tail`, fixed `--version` probes) runs without a prompt. The list is an **allow-list**, so an unrecognised command prompts; a corrupt or missing list makes everything prompt. A command containing a shell operator (`;`, `&`, `\|`, `<`, `>`, a backtick, a newline, `$(`, `${` or `%VAR%`) is never safe, whatever it starts with. |
| FR-PA4 | M | The safe-list is the shipped set plus user additions read **only from Settings** (`safeCommands`), never offered beside a prompt. Each addition is validated on merge: no shell operator, a known match mode (`exact` or `prefix`), 200 characters or fewer, 200 entries or fewer. |
| FR-PA5 | M | Offer **Allow once**, **Allow for this Turn**, and **No** for ordinary Terminal approvals. A Turn grant covers only the same Tool, Conversation, Turn, Project, working directory, and frozen execution policy. It expires on Stop, completion/failure, the next Turn, or live Permission/Project/policy revocation. Browser and child approvals stay once-only. |
| FR-PA6 | M | Show the exact command verbatim, the working directory, the Project and a plain-language risk line, built from an **allow-list** of known fields. Unknown Tool arguments contribute nothing; no secret, environment value or file body may appear, and the Model's own stated reason is never shown. |
| FR-PA7 | M | Correlate every answer with one Conversation, Turn, request id and **action fingerprint** (SHA-256 over Tool, class, Conversation, Turn, command and working directory). A stale, replayed or cross-Conversation answer authorizes nothing, and the first answer wins. |
| FR-PA8 | M | Treat denial as a **structured Tool result the Agent can recover from**, not a failed Turn, carrying the operator's optional free-text note. Stop while approval is pending cancels the wait and runs nothing. |
| FR-PA9 | M | Deny an unanswered request after `approvalTimeoutMinutes` (1-1440, default 15) and release the bridge, so a request nobody answers cannot hold the single Engine Runspace. |
| FR-PA10 | S | Deliver a pending approval to the DeskPilot window **and** the Intercom private chat, and re-deliver it to a browser that reconnects mid-approval. A group chat may answer only when `intercom.groupApproval` is on - a third switch, defaulted off, gated by its own re-confirmation. |
| FR-PA11 | M | Record approval status, once/turn scope, source, and time in Activity; record window decisions in Diagnostics without command arguments. Grant state is in-memory only and is never restored from stored Activity. There is no separate durable approval log. |
...
| FR-BA4 | M | Derive the run's **scope** from hosts the **user** named in their own message, plus the Project's validated `browserDomains`, plus hosts approved during this run. It is **never** seeded from the address the Model chose: doing so gave every Turn one free, unapproved navigation to any host on the internet, which is a complete exfiltration channel because the Model's context holds the conversation and the Workspace Folder path. The user is still never asked to predict a domain list in advance. |
| FR-BA5 | M | Raise an approval card **before the request leaves** for any top-level navigation outside scope, and for any in-scope address whose query or fragment the Model composed rather than took from a link on the page it just read — same-origin egress is the same channel one hop shorter. Show the whole URL including the query string. Approval is for that run only; there is no "always allow" beside the prompt, and durable widening happens only in Settings. |
| FR-BA6 | M | Refuse outright, with **no approvable card**, anything that cannot be scoped or that reaches the local machine: a scheme other than `https` (`file:`, `data:`, `javascript:`, `blob:`, plain `http`), credentials embedded in the URL (including an empty `:@`), any IP literal, single-label hosts, and `.local`/`.localhost` names. The refusal is evaluated **before** the scope match, so a Project entry cannot re-open it. Additionally refuse a navigation whose **peer address** is loopback, RFC1918, link-local or CGNAT, checked after it lands — a public name can hold a private A record, and a pre-flight resolve is a TOCTOU against DNS rebinding. |
| FR-BA7 | M | Enforce policy **below the Model**, inside the supervisor's request interceptor, so it also covers redirect chains, nested frames, pop-ups and sub-resources that a pre-flight check cannot see. Page content may never widen scope. Both enforcement points are held to one shared conformance corpus, which asserts **equality** on every case and the one-way invariant that the supervisor may never be more permissive. Host comparison uses the punycode (IDNA) form on both sides, because the two parsers otherwise disagree about what Chromium will contact — and the approval card would then name a host the browser never visits. |
...
| FR-BA11 | M | Bound the run: actions, navigations, page text, link count, link `href` length, page title length, screenshots, downloads (count **and bytes**), upload size, emitted refusals and per-action wall clock. Stop **and the end of every Turn** close the whole browser process tree — not the start of the next Turn, which would leave a window running an attacker-controlled page for as long as the user does not send another message. |
...
| FR-BW8 | M | Re-check the scope after a control press, because a form that posts to another site is a navigation wearing a button. Bind every write to the page it was approved on: the approved URL travels with the action and the supervisor refuses if the page moved while the card was waiting. |
| FR-BW9 | M | Carry the protocol between the Host Server and the supervisor in **UTF-8**. Inheriting the console code page means the card shows `Straße`, the fingerprint binds `Straße`, and the browser types `Stra?e` — the action performed would not be the action approved, silently, for every non-ASCII value. |
...
| FR-SW10 | S | Support a **file trigger** (`recurrence: onFileChange`): a schedule whose clock is a file appearing in its Project rather than a time of day. It carries a Project-relative `watchGlob`, waits `stabilitySeconds` for the file to stop changing before it counts as complete, and refuses a file over `maxFileBytes` rather than truncating it. It reuses the same queue, coalescing, collision policy, claim and run history as a timed schedule; there is no second dispatcher. |
| FR-SW11 | M | A file trigger is **locked to the `safe` permission mode**. A timed schedule runs at a moment the operator chose; a trigger runs at a moment whatever wrote the file chose, so it may not hold Terminal authority until per-call approval exists (`specs/120`). The control is removed in the UI and the value is refused by the API. |
| FR-SW12 | M | Treat the triggering file as **data**. The stored prompt is never rewritten; the Project-relative path is appended to the Turn's prompt with an explicit instruction to treat the contents as data rather than instructions, and the file's contents never enter the prompt. |
| FR-SW13 | M | Confine every event to the Project **by shape and by resolution**: a rooted or traversing `watchGlob` is refused when the schedule is saved, and the scan prunes reparse points rather than following them. Deleted files are forgotten so the trigger's state stays bounded, and a file already acted on does not fire again until its content changes. |
