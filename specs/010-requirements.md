...
| ID | Priority | Requirement |
| --- | --- | --- |
| FR-PA1 | S | When approval is active, run `Invoke-Shp` with **`-DisableTerminal`** and register a DeskPilot-owned Terminal User Tool instead. Removing the built-in from the offered tool set is what leaves the owned Tool as the only path the Model is shown. **Correction 2026-09-03: the owned Tool must not be named `run_command`.** `$terminalEnabled` gates the offered definition but not the Engine's dispatch clause, and User Tools are dispatched only from that switch's `default`, so a User Tool sharing the built-in name is never invoked and the command runs ungated (decision 0001). The owned Tool therefore needs a distinct name plus a `Set-ShpToolPolicy` denial closing the built-in path. Approval is active only when `perCallApproval`, the Terminal Permission and the Your Tools Permission are all on. |
