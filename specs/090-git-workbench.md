...
| While the Turn is still running | None of the four can answer "what is it writing *now*" — they all need the write to have happened. The Engine announces each `write_file` before it performs it, so the Changes card element carries a live row per file from that announcement, then is replaced by the reviewed card on `done`. Intent, not record: no counts (measuring one would cost a Git read per write on the thread holding the stream open) and no diff (there is nothing to diff yet). |
