...
| Question nonce | Each forwarded question records the Telegram `message_id` it was sent as. An answer is accepted when it is a **reply to that message** and the question is still pending and unexpired (`questionTimeoutMinutes`, default 60). A validated **Something else** tap additionally arms one ordinary message from that same chat as free text. |
...
- Every list of options also carries **`Something else - type it`**. Tapping it
  makes the next ordinary message in that chat the answer, taken verbatim - so a
  literal "2" stays "2" rather than selecting the second choice. This is a
  deliberate divergence from the browser wizard, which honours
  `allowFreeformInput` strictly:
...
4. The operator replies to that message on their phone, or taps **Something
  else** and types the answer as their next message.
5. The next pump tick matches either the reply's
  `reply_to_message.message_id` or the armed free-text state in the same chat,
  and calls `SubmitAnswer` on the bridge.
