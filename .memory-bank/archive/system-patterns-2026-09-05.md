...
- **HTTPS allow-lists require protocol and pre-egress authority checks.** A
  host/443 allowance is not HTTPS enforcement. The Terminal proxy establishes
  client-side TLS first, checks the decrypted HTTP authority, and accepts
  non-CONNECT requests only over an encrypted HTTPS connection. Tests must
  distinguish a proxy policy denial from a remote 400 or failed TLS handshake:
  both were real outward contacts in the first implementation. Network packet
  rules remain necessary because proxy environment variables can be ignored.
- **Stop owns the whole execution environment.** Killing a Docker CLI client
  does not kill its container. Terminal cleanup removes and verifies the
  command/proxy pair, and a required-proxy crash stops the command rather than
  leaving it running with a dead dependency. Runtime source changes invalidate
  prepared image provenance; never edit those sources during a proof run.
