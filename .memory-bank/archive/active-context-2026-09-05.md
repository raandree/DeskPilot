...
**Isolated Terminal implementation is complete; final verification is in progress.** The operator
retained working nonempty outbound allow-lists, rejected an offline-only slice,
and directed continuation to completion after the full-network design was
described. Decision 0001 records the scope, temporary in-container CA, protected
network namespace, and unchanged prohibition on host trust changes or pushes.

Implemented and measured so far:

- Real `Invoke-Shp` dispatch tests prove disabled native Terminal refusal and
  owned Tool dispatch against the staged ShellPilot 0.4.1.
- Isolated ownership stays active with Local approval off or User Tools off;
  Local behavior is unchanged. The two new checks were red, then green.
- `terminalExecution` Settings have conservative defaults, strict validation,
  immutable merges, exact host names, and environment names/secret markers only.
  All 24 policy checks were red before implementation. Policy plus approval:
  **89 passed, 0 failed, 0 skipped**, using temporary Pester 5.7.1.
- A real-Engine integration test changed from reporting Windows to reporting
  Linux through the registered Tool. The first Docker controller runs pinned,
  unprivileged, bounded, network-off containers and verifies removal.
- Nonempty exact HTTPS allow-lists now work through a disposable Squid proxy
  and namespace-local packet rules. Client-first TLS is essential: the first
  server-first implementation attempted upstream TLS before denying a spoofed
  HTTP Host. The corrected positive and hostile-Host tests both pass.
- The release runtime uses a digest-pinned base plus the SHA-256-verified
  official PowerShell 7.6.5 archive and a recorded package/image inventory.
- Turn preflight, Stop, frozen approval/Activity metadata, pending change
  accounting, Settings, Diagnostics, asynchronous preparation, and removal are
  implemented. Real-container integration: **29 passed, 0 failed, 0 skipped**.
  Desktop/mobile policy-saving and overflow checks pass in Playwright.
- Full Sampler before final proxy adjustments: **2266 passed**, 16 tasks,
  zero errors/warnings. A final full run and real HTTP/Model smoke are next.
- Independent security review found **0 Blockers, 1 Major**. MAJOR-01 was
  reproduced: plain HTTP-over-443 reached the remote server, and direct HTTPS
  without CONNECT returned 200. Explicit CONNECT/encrypted-connection/HTTPS
  ACLs now deny both with 403 while normal HTTPS works. All four protocol
  regression checks pass. The pinned Squid 5 uses `connections_encrypted`, not
  the unsupported `ssl::bump_mode` ACL.
- A separate crash proof found that commands survived a proxy crash. The
  controller now monitors its required proxy and terminates on dependency loss;
  the same proof is green with cleanup verified.

Work is uncommitted on `ai/isolation-dependency-decision`. Pester 5.7.1 is under
the user's temporary `deskpilot-pester5` directory; the normal dependency cache
currently resolves Pester 6.1.0. Test logs use unique `deskpilot-*.log` names in
the user temporary directory. No production Settings were changed.

Remaining release dependency: staged ShellPilot 0.4.1 enforces disabled Tools;
installed 0.4.0 does not. A compatible Engine must be obtainable for a clean
installation. No publish or push is authorized.

### Prior dependency investigation
