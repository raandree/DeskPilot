...
| Intercom | DeskPilot's two-way link to the operator's phone: it pushes a blocked question, a finished or failed job, and a stall warning to an allow-listed Telegram chat, and accepts answers, instructions and commands back. The operator's own chat is always the primary one; a shared group chat may be allow-listed alongside it, off by default and behind two switches, and everyone in that group then holds the operator's authority. Off by default, and inert for any Project that has not opted in. | beacon, relay, remote control, bot, notifier, companion |
