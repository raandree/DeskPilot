...
| Date | Milestone |
| --- | --- |
| 2026-09-25 | Complete the independent review of DeskPilot `a7a729e` against ShellPilot `08a4a22`: request changes. Confirm new High eval-launcher argument injection by actual manifest/allocation plus inert AST; no injected command executed. Correct Memory framing to pre-existing Minor/Low with no Model compromise proof. Confirm legacy callback mismatch and pre-existing source-marker false negatives on both gated Terminal paths. Native Engine controls pass; 174 integration tests pass and five marker-selected tests skip. No production fix, default Engine change, push or Merge; review artifacts retained. |
